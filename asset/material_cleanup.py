...
# 快速从 section 映射中派生使用的材质槽索引（避免重复遍历）
def derive_used_indices_from_map(section_slot_map, num_materials):
    used = set()
    for _, _, slot in section_slot_map:
        if 0 <= slot < num_materials:
            used.add(slot)
    return used

# 控制更新日志的详细程度（大项目可关闭以提速）
VERBOSE_SECTION_UPDATE_LOG = False
...
def remove_unused_material_slots(selected_only=True):
    ...
    for mesh in assets:
        ...
        # 优先用 section 映射直接得到使用的材质槽索引，避免二次遍历
        used_indices = derive_used_indices_from_map(section_slot_map, num_materials)
        if not used_indices:
            # 映射为空或无有效索引时，回退到旧方法
